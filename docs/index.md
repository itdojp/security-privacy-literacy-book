...
- **認証・セッションは専門書の責務と分ける**: 本書では MFA、共有アカウント禁止、セッション失効、ログマスキングの判断基準に絞る。OAuth / OIDC / JWT / Cookie / CSRF / CORS の実装判断は [実践 認証認可システム設計](https://itdojp.github.io/practical-auth-book/) 側のレビューゲートを参照する。
